...
_Please note that an additional GlobalVariable is present only in memory during a run of test; it is not persisted into any `glbl` file in the `Profiles` directory._
